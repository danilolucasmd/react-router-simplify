      ...
      <Fragment>
        ...
        <Switch>
          {(children.type.routes || []).map((route, index) => {
            ...
            let Component = CompEl.type.routes && CompEl.type.routes.length ? this.renderRoutes(CompEl, auth, path) : CompEl;
            ...
            return (
              <Route
                ...
                path={path}
                exact={route.exact}
                render={() => this.handleAuthentication(Component, route, auth)}
              />
            );
          })}
        </Switch>
